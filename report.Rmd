...
librarian::shelf(tidyverse, haven, kableExtra, modelr, rsample, stargazer)
...
```{r tree, file = "code/randomForest_new.R", echo = FALSE, message=FALSE, warning=FALSE, results = 'asis'}
tree_vip = read_csv("figures/tree_vip.csv")
colnames(tree_vip) = "Importance"
tree_vip %>% order(-Importance)
kbl(tree_vip, booktabs = T, format = "latex", caption = "CART: Variable Importance") %>% kable_classic(full_width = F, html_font = "Cambria") %>% kable_styling(latex_options = 'hold_position')

```

### Random Forest
To further deepen the model, we next used a random forest of 50 trees to identify the features most predictive of an over or under performing district.
```{r}

```
![Fig 2: Partial Dependence of Fund Balance](figures/pdp_fundbal.png)
![Fig 3: Partial Dependence of Operating Revenue](figures/pdp_oprev.png)
![Fig 4: Partial Dependence of Student Percentage Asian](figures/pdp_stasian.png)
![Fig 5: Partial Dependence of Student Percentage Hispanic](figures/pdp_sthisp.png)
![Fig 6: Partial Dependence of Teacher Percentage Black](figures/pdp_teachblack.png)
