...
gridExtra::grid.arrange(arrangeGrob(st_pct_ecodis_plot_out, 
                                    FreeElig_mean_plot_out,
                                    avg_salary_central_plot_out,
                                    exp_pct_na_plot_out,
                                    st_to_teach_minority_plot_out,
                                    description_out),
                        top="Partial Dependence Plots for All School Performance")

################ RF outcomes pred v actual
yhat_out =  predict(rf_out,newdata=mob_test)
mob_test =  cbind(mob_test,yhat_out)

ggplot(mob_test, aes(x=yhat_out,y=PC_outcome))+
  geom_jitter() +
  geom_abline(slope = 1,intercept = 0,color="black",linetype="dashed") +
  geom_smooth() +
  xlab("Predicted Outcome (randomForest)") +
  ylab("Actual Outcome (PCA)")
